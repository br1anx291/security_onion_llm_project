...
def main_realtime():
    """
    Hàm chính để chạy script liên tục, theo dõi và đồng bộ log Zeek
    gần như thời gian thực.
    """
    # Khoảng thời gian nghỉ giữa các lần đồng bộ (tính bằng giây)
    SYNC_INTERVAL = 5
    ...
    try:
        ...
        while True:
            try:
                ...
                if not folders_to_check:
                    ...
                    for folder_name in folders_to_check:
                        # Tải các file .log.gz mới hoặc đã thay đổi
                        downloaded_files = fetch_logs_from_folder(folder_name, ssh, sftp)
                        
                        # Nếu có file mới được tải, tiến hành giải nén
                        if downloaded_files:
                            logging.info(f"Downloaded {len(downloaded_files)} new files from {folder_name}. Decompressing...")
                            decompress_logs_only(folder_name, downloaded_files)
                        # else:
                        #     logging.info(f"No new files to download from {folder_name}.")

                logging.info(f"✅ Sync cycle complete. Waiting for {SYNC_INTERVAL} seconds...")
                ...
                logging.error(f"An error occurred during sync cycle: {e}")
                logging.info(f"Attempting to continue in {SYNC_INTERVAL} seconds...")
                time.sleep(SYNC_INTERVAL)
        ...
        if ssh:
            ssh.close()
        logging.info("✅ SSH and SFTP connections closed. Zeek log pipeline stopped.")
